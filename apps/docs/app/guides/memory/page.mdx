...
### Manual Memories (via Skills)

If you're using Celune via Claude Code, you can create memories directly with the /remember skill:

```
/remember "The staging environment uses a separate Supabase project. Always check NEXT_PUBLIC_SUPABASE_URL before running migrations."
```
...
You can query memory explicitly from Claude Code using the /recall skill:

```
/recall "How does staging environment differ from production?"
```
...
| Plan                          | Memory Entries       |
| ----------------------------- | -------------------- |
| Builder (Free)                | 0 (seed & core only) |
| Pro ($19/mo)                  | Unlimited            |
| Unlimited ($49/mo + $10/seat) | Unlimited            |
...
- [MCP Setup](/guides/mcp-setup) - Use /remember and /recall from Claude Code.
