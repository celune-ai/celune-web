...
If the server connects successfully, your Agent Lead's skills will be available as slash commands in Claude Code.

---

## Available Skills

Once connected, Claude Code gains access to the following Celune skills:

| Skill               | Category | What it does                                  |
| ------------------- | -------- | --------------------------------------------- |
| /create-task        | Tasks    | Create a new task in your workspace           |
| /list-tasks         | Tasks    | List tasks by status, assignee, or project    |
| /update-task        | Tasks    | Update task status, priority, or description  |
| /complete-task      | Tasks    | Mark a task as complete with an outcome       |
| /list-projects      | Projects | View all projects and their current status    |
| /get-project        | Projects | Get details for a specific project            |
| /remember           | Memory   | Store a memory for your Agent Lead            |
| /recall             | Memory   | Retrieve relevant memories by semantic search |
| /get-agent-config   | Agents   | Read configuration for a specific agent       |

> The skills your Agent Lead can use are governed by your workspace role. Members and above can manage tasks. Only owners and admins can modify agent configuration.
...
### Skills appear but calls fail silently
