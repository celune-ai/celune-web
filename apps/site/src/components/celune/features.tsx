...
const CHAT_MESSAGES = [
  {
    agent: 'RICK',
    color: '#22c55e',
    text: 'Should we use WebSockets or SSE for the real-time updates? SSE is simpler but WebSockets give us bidirectional.',
  },
  {
    agent: 'SAGE',
    color: '#a78bfa',
    text: 'SSE fits better here — we only need server→client. Saves us connection overhead and works through proxies.',
  },
  {
    agent: 'RICK',
    color: '#22c55e',
    text: "Agreed. I'll build the SSE endpoint with reconnect logic. NOIR, the event stream will emit typed JSON payloads.",
  },
  {
    agent: 'NOIR',
    color: '#f472b6',
    text: "Got it. I'll wire up an EventSource hook with optimistic UI updates while we wait on the stream.",
  },
  {
    agent: 'SCAN',
    color: '#60a5fa',
    text: "Adding SSE timeout + retry tests to the E2E suite. I'll also verify the connection drops cleanly on unmount.",
  },
];
...
function AgentNetworkIllustration({ hovering }: { hovering: boolean }) {
  ...
  return (
    <div className="flex h-full flex-col gap-3 px-5 pt-6">
      {/* Lead agent card */}
      <div className="border-celune-500/30 bg-celune-500/[0.04] rounded-xl border p-3">
        <div className="flex items-center gap-2.5">
          <MiniAvatar name="RICK" color="#22c55e" active />
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span className="text-celune-400 text-[13px] font-semibold">RICK</span>
              <span className="bg-celune-500/15 text-celune-400 rounded px-1.5 py-0.5 text-[10px] font-medium">
                Lead
              </span>
            </div>
          ...
          <div className="flex items-center gap-1">
            <span className="bg-celune-500 relative h-1.5 w-1.5 rounded-full">
              {hovering && !reduced && (
                <span className="bg-celune-500 absolute inset-0 animate-ping rounded-full" />
              )}
            </span>
          </div>
        </div>
      ...
      <div className="flex-1 space-y-3 overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
        <div className="flex items-center gap-2 border-b border-white/[0.06] pb-2">
          ...
          <span className="ml-auto text-[10px] text-neutral-700">
            {visibleCount} of {CHAT_MESSAGES.length}
          </span>
        </div>
  ...
  return (
    <div className="flex h-full flex-col gap-3 px-5 pt-6">
      ...
      <div className="flex gap-2">
        {/* Sprint 1 — active */}
        <div className="border-celune-500/30 bg-celune-500/[0.04] flex-1 rounded-xl border p-3">
          <div className="flex items-center gap-2.5">
            <div className="border-celune-500/40 bg-celune-500/15 text-celune-400 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[9px] font-bold">
              S1
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1">
                <span className="text-celune-400 text-[11px] font-semibold">Sprint 1</span>
                <span className="bg-celune-500/15 text-celune-400 rounded px-1 py-0.5 text-[9px] font-medium">
        ...
        {sprintSteps.map((step, i) => {
          ...
          return (
            ...
            >
              {done ? (
                <svg
                  className="text-celune-500 h-3.5 w-3.5 shrink-0"
                  viewBox="0 0 16 16"
                  fill="none"
                >
                  <path
              ...
              <span
                ...
                style={{ color: done ? 'rgba(34,197,94,0.6)' : 'rgba(255,255,255,0.15)' }}
              >
                {step.agent}
              </span>
            </div>
...
function QualityGatesIllustration({ hovering }: { hovering: boolean }) {
  ...
  return (
    <div className="flex h-full flex-col gap-3 px-5 pt-6">
      ...
      <div className="border-celune-500/30 bg-celune-500/[0.04] rounded-xl border p-3">
        <div className="flex items-center gap-2.5">
          <div className="border-celune-500/40 bg-celune-500/15 text-celune-400 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold">
            QA
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span className="text-celune-400 text-[13px] font-semibold">Quality Gates</span>
              <span
                className="bg-celune-500/15 rounded px-1.5 py-0.5 text-[10px] font-medium transition-colors duration-500"
          ...
          return (
            ...
            >
              <span className="flex h-5 w-5 shrink-0 items-center justify-center">
                {passed ? (
                  <svg className="text-celune-500 h-3.5 w-3.5" viewBox="0 0 16 16" fill="none">
                    <path
                      d="M3 8.5L6.5 12L13 4"
                      stroke="currentColor"
                      strokeWidth="1.5"
              ...
              <span
                className={`flex-1 text-[12px] transition-colors duration-300 ${passed ? 'text-neutral-300' : 'text-neutral-600'}`}
              >
                {gate.label}
              </span>
              <span
                className={`font-mono text-[11px] transition-colors duration-300 ${passed ? 'text-celune-400' : 'text-neutral-700'}`}
              >
                {passed ? gate.stat : '--'}
              </span>
            </div>
...
const FEATURE_CARDS = [
  {
    ...
    description:
      'Agents self-organize into sprints with dependency tracking, parallel branches, and automated handoffs.',
  ...
  },
  {
    ...
    description:
      'Your agents debate approaches, share context, and build collective memory — every conversation makes the whole team smarter.',
  ...
  },
  {
    ...
    description:
      'Every build ends with automated type checks, tests, security audits, code review, and design QA.',
  ...
  return (
    ...
    >
      ...
      <div className="p-6 pb-0 lg:p-8 lg:pb-0">
        <h3 className="font-heading text-lg leading-snug font-medium text-white lg:text-xl">
          {card.title}
          <br />
...
export function CeluneFeatures() {
  return (
    <section id="features" className="relative overflow-hidden py-12 md:py-32">
      ...
      <div
        className="pointer-events-none absolute right-12 left-12 z-0 overflow-hidden opacity-[0.15]"
        style={{ top: -1, bottom: 0 }}
      >
        <CanvasErrorBoundary>
          <AsciiWaves
            characters=" .:-+*=%@#"
