...
<ProductScreenshot
  ...
  alt="Celune memory system interface showing typed memory categories with entries for user preferences, project facts, feedback, and references"
  width={1200}
  height={750}
  layout="full"
  caption="Typed memory categories in practice — each entry is tagged by type so agents load only what is relevant to the current task."
/>
...
<ProductScreenshot
  ...
  alt="Celune agent team cards showing RICK, SAGE, NOIR, SCAN, and DELV agents — each with their own identity and shared memory access"
  width={1200}
  height={750}
  layout="float-right"
  caption="Multiple agents sharing a memory store — each has its own identity namespace while accessing shared project-level memories."
/>
